...
def _sev_emoji(sev: str) -> str:
    return {"critical": "🔴", "warning": "🟡", "normal": "🟢"}.get(sev, "⚪")
...
def _weibull_projection(asset_type: str, cycle: int | float, rul_days: float,
    ...
    """Dual-path degradation: 'do nothing' vs 'act now' — the key ROI visual."""
    cycle = int(cycle)
    ...
    # Do nothing — forward to collapse
    ...
    # Act now — maintenance in 7 days, then partial reset to 82 % of new
    ...
    fig.add_trace(go.Scatter(x=act_cycles, y=act_vals, name="GötzeEngine — Act now",
    ...
    fig.add_vline(x=cycle, line_color="rgba(255,255,255,0.4)", line_dash="dot",
                  annotation_text="Now", annotation_position="top")
    fig.add_vrect(x0=maint_c - 2, x1=maint_c + 20,
                  fillcolor="rgba(76,120,168,0.10)", layer="below", line_width=0)
    ...
    fig.update_layout(
        ...
        yaxis=dict(range=[0, 108], gridcolor="#222", title="Health Score"),
...
def _signal_waterfall(cycle: int | float, flagged_signals: list[str]) -> go.Figure:
    """5-signal time-series waterfall. Anomalous signals rendered in red."""
    cycle = int(cycle)
    ...
    for i, sig in enumerate(KEY, start=1):
        ...
        for j, c in enumerate(cycles):
            ...
            if flagged and j >= hist - 14:   # anomaly spike in last 14 cycles
    ...
    fig.update_yaxes(gridcolor="#222", showticklabels=False)
    return fig


def _savings_timeline(rul_days: float, base: float = 180_000) -> go.Figure:
    """Savings erode every day we delay — the urgency countdown."""
    ...
    fig.update_layout(
        ...
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="white",
        yaxis=dict(range=[0, base * 1.15], gridcolor="#222", tickformat="$,.0f"),
...
def _generate_live_reading(scenario_key: str, cycle: int,
    ...
    """Build a SensorReading for the current live cycle, optionally injecting a failure."""
    base = SCENARIOS[scenario_key]["reading"]
    rng  = np.random.default_rng(cycle % (2**31))
...
if plant_info:
    st.sidebar.markdown(
        f"**{plant_info.name}**  \n"
        f"<span style='color:#888;font-size:12px'>{plant_info.location} · {plant_info.sector}</span>",
...
if live_toggle != st.session_state.live_mode:
    ...
    st.session_state.live_cycle = SCENARIOS[selected]["reading"].cycle

if st.session_state.live_mode:
    st.sidebar.markdown("🔴 **LIVE** — data streaming")
    ...
    st.sidebar.markdown(f"Cycle: **{st.session_state.live_cycle}**")

st.sidebar.divider()

# Failure Injector
st.sidebar.header("Failure Injector")
...
# Manual run button (shown in non-live mode)
...
if st.session_state.live_mode:
    ...
    st.session_state.live_cycle = cycle + 1
...
with status_col:
    if st.session_state.live_mode:
        st.markdown(
            '<div style="margin-top:20px;padding:8px 14px;background:#FF4B4B22;border:1px solid #FF4B4B;'
            'border-radius:8px;color:#FF4B4B;font-weight:bold;text-align:center">🔴 LIVE</div>',
            unsafe_allow_html=True,
        )

if result is None:
    st.info("Select a scenario in the sidebar and click **Run Analysis** to start, or enable **Live Mode**.")
    st.stop()
...
col3.metric("Severity", _sev_emoji(r.anomaly_severity) + " " + r.anomaly_severity.upper())
col4.metric("Downtime Saved", f"${brief.downtime_saved_estimate:,.0f}",
            delta=f"GötzeEngine pending" if brief.gotze_pending else "No action needed")
...
with gcol:
    st.plotly_chart(_gauge(r.asset_id, r.health_report.health_score, r.health_report.rul_days),
...
# ── signal waterfall + savings timeline ───────────────────────────────────────
w_col, s_col = st.columns([2, 1])
with w_col:
    st.markdown("**Signal Anomaly Waterfall** — last 45 cycles")
    if r.flagged_signals:
        st.plotly_chart(_signal_waterfall(_proj_cycle, r.flagged_signals),
                        use_container_width=True)
    else:
...
if r.flagged_signals:
    html = " ".join(
        f'<span style="background:#FF4B4B;padding:3px 10px;border-radius:4px;'
        f'margin:3px;font-size:13px;display:inline-block">{s}</span>'
...
if r.gotze_triggered:
    if st.session_state.approval_submitted:
        ...
        st.warning(
            f"GötzeEngine fired — recommended: **{r.gotze_decision.top_intervention.replace('_',' ')}** "
